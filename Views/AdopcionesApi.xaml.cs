...
namespace SignInAndSignUpUI.Views;

public partial class AdopcionesApi : ContentPage
{
    private const string apiUrl = "https://dog.ceo/api/breeds/list";
    private SQLiteConnection database;
    private List<string> perrosGuardados = new List<string>();
    public AdopcionesApi()
	{
		InitializeComponent();
	}
    ...
    private async void GuardarPerro_Clicked(object sender, EventArgs e)
    {
        string breedName = BreedsLabel.Text.Trim();

        if (!string.IsNullOrEmpty(breedName))
        {
            // Verificar si el perro ya ha sido guardado previamente
            if (!perrosGuardados.Contains(breedName))
            {
                // Agregar el nombre del perro a la lista y mostrarlo en el XAML
                perrosGuardados.Add(breedName);
                MostrarPerrosGuardados();

                // Mostrar un mensaje de confirmación
                await DisplayAlert("Éxito", $"Perro '{breedName}' guardado correctamente", "OK");
            }
            else
            {
                await DisplayAlert("Advertencia", $"El perro '{breedName}' ya ha sido guardado previamente", "OK");
            }
        }
        else
        {
            await DisplayAlert("Error", "No hay información para guardar", "OK");
        }
    }

    private void MostrarPerrosGuardados()
    {
        PerrosGuardadosLabel.Text = string.Join(", ", perrosGuardados);
    }
}
